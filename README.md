...
## 量化指标 (真实测量, 2026-07-03)

语料: 2705 chunks (西门子 S7-1200 中文系统手册 1156 页 + 三菱 FX3U 手册) · CPU 推理 (无 GPU)

| 检索模式 | Top-3 命中率 | 平均延迟 |
|---|---|---|
| 纯向量 (baseline) | 80.8% | 124ms |
| 混合检索 (BM25+向量+RRF) | 73.1% | 130ms |
| 混合 + BGE-Reranker | **96.2%** | 3483ms |

**两个诚实的发现**(比全线飘红更有讨论价值):

1. **混合检索单独用反而降了 7.7 个点** — BM25 通道的噪声在 RRF 排名中挤占 Top-3。
   但它的真正价值是**扩大候选池**: Reranker 正是在混合召回的 20 条候选上精排,
   才把命中率推到 96.2%(比纯向量提升 **15.4 个百分点**)。粗排负责"捞得全",
   精排负责"排得准" — 两级流水线各司其职。
2. **Reranker 延迟 3.4s 是 CPU 跑 Cross-Encoder 的代价** — 每条查询要对 20 个
   (query, chunk) 对逐一推理。换 GPU 或减小候选池可显著降低(见 DECISIONS #5)。
